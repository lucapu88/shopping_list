...
<template>
	<ListIstructionAccordion show-list-instructions-input="categoriesInfo" :istructions-text="languages.helperDescription.categories" :select-deselect-arrow="settings.categoriesInfo && settings.section === categoriesInfo" />
	<ul class="helper-list" v-if="settings.categoriesInfo && settings.section === categoriesInfo">
		<li>
			...
			<span :class="theme.elegantTheme ? 'elegant-selected' : 'selected'"> {{ languages.helperDescription.addCategory.part6 }}. </span>
		...
		<Tutorial v-if="settings.video && settings.feature === categories" :features="categories" />
	</ul>
</template>
